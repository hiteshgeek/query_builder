<?php
/**
 * UPDATE Query Execution API
 *
 * Executes UPDATE queries with proper validation and security
...
if (empty($input['set']) || !is_array($input['set'])) {
    json_error('At least one column to update is required', 400);
}
...
try {
    ...
    foreach ($setValues as $colName => $valObj) {
        if (!isset($columnMap[$colName])) {
            json_error("Unknown column '$colName'", 400);
        }

        if (is_array($valObj) && isset($valObj['isNull']) && $valObj['isNull']) {
            $setClauses[] = "`$colName` = NULL";
        } else {
            $value = is_array($valObj) ? ($valObj['value'] ?? '') : $valObj;
            $setClauses[] = "`$colName` = ?";
            $setParams[] = $value;
        }
    }
    ...
    if (strpos($errorMessage, 'foreign key constraint') !== false) {
        json_error('Foreign key constraint violation', 400);
    } elseif (strpos($errorMessage, 'Duplicate entry') !== false) {
        json_error('Duplicate entry: ' . $errorMessage, 409);
    } else {
